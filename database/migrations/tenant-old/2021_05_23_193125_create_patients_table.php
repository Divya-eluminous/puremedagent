...
class CreatePatientsTable extends Migration {
	...
	public function up()
	{
		Schema::create('patients', function(Blueprint $table)
		{
			...
			$table->string('family_name', 50)->nullable()->index('family_name_2');
			$table->string('first_name')->nullable()->index('first_name_2');
			...
			$table->string('age', 100)->default('\'0\'')->index('age');
			...
			$table->timestamps();
			$table->softDeletes();
			$table->enum('patient_status_flag', array('0','1'));
			$table->enum('reminder_active', array('0','1'))->default('1');
			$table->text('note_report_request', 65535)->nullable();
			$table->enum('note_report_request_flag', array('0','1','2'));
			$table->string('street_no')->nullable();
			$table->index(['first_name','family_name'], 'first_name');
