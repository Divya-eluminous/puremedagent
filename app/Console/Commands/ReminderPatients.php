...
class ReminderPatients extends Command
{
    /**
    ...
    protected $signature = 'reminderpatients:daily {--tenant_id=}';
        ...
        if(!empty($is_reminder_execute))
        {
             //client 33890
               ...
               /* $collections = DB::connection('tenant')->table('patient_has_service_reminder')
                ...
                //changed on 17-sept-25 //take those records who does not have record in reminder patient table
                                        ...
                                        if($getLastDateDescOrder->type=="age"){
                                            ...
                                            if($appointmentId==0)
                                            {
                                                ...
                                                //Check status is active or not
                                                 ...
                                                 /*if(((strtotime($reminderDate) < strtotime($currentDate)) && $getLastDateDescOrder->cycle_no<2) || $getLastDateDescOrder->appointment_id==0)
                                                {
                                                    dump("in 1st");
                                                    $status =  'aktiv';
                                                }
                                                ...
                                                if($getLastDateDescOrder->reminder_status=='ignore' && $getLastDateDescOrder->status!='deactivate')
                                                {
                                                ...
                                                if($getLastDateDescOrder->appointment_id==0 && $getLastDateDescOrder->status=='deactivate')
                                                {
                                                    $status ='inaktiv';
                                                }*/
                                            ...
                                            {
                                               // log::info("appointmentId is not 0");

                                                //commented on 21-apr-25
                                                //get last of 1st row date

                                                /*$getLastFirstDateAscOrder =  DB::connection('tenant')->table('patient_has_service_reminder')
                                                ->where('patient_has_service_reminder.reminder_status','Set')
                                                ->where('patient_has_service_reminder.patient_id',$patientId)
                                                ->where('patient_has_service_reminder.service_id',$serviceId)
                                                ->where('appointment_id',$getLastDateDescOrder->appointment_id)
                                                ->where('patient_has_service_reminder.type',$type)
                                                ->whereNull('patient_has_service_reminder.deleted_at') 
                                                ->orderBy('patient_has_service_reminder.id','asc')
                                                ->first();*/
                                                ...
                                                //commented on 21-apr-25
                                               /* if(isset($getLastFirstDateAscOrder) && !empty($getLastFirstDateAscOrder))
                                                {

                                                     dump("reminder_date");
                                                     dump($getLastFirstDateAscOrder->reminder_date); 

                                                     dump("appointment_id");
                                                     dump($getLastFirstDateAscOrder->appointment_id); 

                                                    if((strtotime($getLastFirstDateAscOrder->reminder_date) < strtotime($currentDate)) || ((strtotime($getLastFirstDateAscOrder->reminder_date) > strtotime($currentDate)) &&  $getLastFirstDateAscOrder->appointment_id!=0 && $getLastFirstDateAscOrder->cycle_no!=1)  || $getLastFirstDateAscOrder->appointment_id==0)
                                                    {
                                                         dump("in status active condition..");
                                                        $status =  'aktiv';
                                                    }

                                                    if($getLastFirstDateAscOrder->status == 'deactivate'){
                                                        $status = 'inaktiv';
                                                    }

                                                    if($getLastFirstDateAscOrder->reminder_status=='ignore' && $getLastFirstDateAscOrder->status!='deactivate')
                                                    {
                                                        // $status ='inaktiv';//commented on 17-apr-25
                                                        $status='ignored';//added on 17-apr-25
                                                    } 
                                                    ...
                                                    if($getLastFirstDateAscOrder->appointment_id==0 && $getLastFirstDateAscOrder->status=='deactivate')
                                                    {
                                                        $status ='inaktiv';
                                                    }
