				...
				try 
				{
					...
					else
					{
						...
						if ($collection) 
			            {
			            	...
			            	$data[] = $collection->only(['first_name','family_name','email','country_code', 'mobile_no','age','login_type','postal_code','birth_date']);
			    ...
			    {		
					...
					{*/
		                ...
		                if($collection)
		                {
							...
							//Log::info('edit master data(_oldPatient) line number: 1660 . patient Name :' .$collection->first_name.' '.$collection->family_name);
							$log_id = $collection->id;
							$collection = $new_data->only(['first_name','family_name','email','country_code','mobile_no','birth_date','age','road','street_no','postal_code','place']);
		                	$message = __('api.PATIENT_UPDATE_SUCCESS');
			          		$data[] = $collection;
			          		self::_createLog('editMasterData',$log_id,'info');
			          		$this->ActivityLogModel->addApiLog('Edit Master Data','Edit master data','Update',$oldData,$data[0]);
			...
			try {
					...
					if(!empty($valid_tab_pin)){
						...
						if($valid_tab_pin->setting_value == $tab_pin){
			                ...
			                if(!empty($collection) && ($collection->count() > 0)){
			                	$log_id = $collection->id;
								$collection = $collection->only(['id','old_id','first_name','family_name','email','country_code','mobile_no','birth_date','age','road','street_no','place','postal_code','gdpr','social_security_number']);
					...
					if(!empty($valid_tab_pin)){
						...
						if($valid_tab_pin->setting_value == $tab_pin){
							...
							if(!empty($collectionPatient) && ($collectionPatient->count() > 0)){
								...
								$collection = $collectionPatient->only(['id','old_id','first_name','family_name','email','country_code','mobile_no','birth_date','age','road','street_no','place','postal_code','gdpr','social_security_number']);
