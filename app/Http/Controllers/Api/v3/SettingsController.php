...
use App\Models\AdminUserModel; //added on 9-july-25 for #368
...
class SettingsController extends BaseController
{
	...
	public function __construct(
        ...
        AdminUserModel $AdminUserModel //added on 9-july-25
    ...
    )
    {
        ...
        $this->AdminUserModel = $AdminUserModel;//added on 9-july-25
            ...
            if((!empty($getAppointments) && sizeof($getAppointments) > 0) && $getAppointments[0]->services!=""){
               ...
               if(empty($getSameAppointmentsType)){
                ...
                // else $data  = $getSameAppointmentsType;
                     ...
                     $data  = $getSameAppointmentsType;
                }

                $status  = true;
                $message = __('api.DATA_FOUND_SUCCESS');

                // start #368 added on 9-july-25
                if(isset($data) && !empty($data) && isset($doctor_id))
                {
                    $app_type_name = $this->AppointmentTypesModel->where('name',$data['name'])->first();
                    if(isset($app_type_name) && !empty($app_type_name))
                    {

                            $userRestrictedAppoinmentType = $this->AdminUserModel
                                ->with(['userHasAppointmentTypes'=>function($query) use($app_type_name)
                                {
                                         $query->where('appointment_type_id',$app_type_name->id);
                                    }])
                                ->find($doctor_id);
                              

                            if (isset($userRestrictedAppoinmentType->userHasAppointmentTypes) &&
                                !$userRestrictedAppoinmentType->userHasAppointmentTypes->isEmpty()
                            ) 
                            {
                ...
                // end #368 added on 9-july-25 



                //commented below lines on 9-july-25 and added above
    ...
    public function getCycleCalenderSetting(){
        ...
        try{
            $collections = collect([]);   
            $collections = $this->BaseModel
                                ->where('setting_key','=','SHOW_CYCLE_CALENDAR_IN_APP')
                                ->whereStatus(1)
                                ->get();
                ...
                if(!empty($collections)){
                    ...
                    foreach ($collections as $key=>$collection) {
                            ...
                            foreach ($logged_out_images as $image_key=>$logged_out_image) {
                                $data[$image_key]['id']  = $collection->id;
                                $data[$image_key]['setting_key']  = $collection->setting_key;
                                $data[$image_key]['setting_value']  = $collection->setting_value;
                                $data[$image_key]['description']  = $collection->description;
                                $data[$image_key]['status']  = $collection->status;
                            }
