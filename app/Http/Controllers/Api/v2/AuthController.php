...
class AuthController extends BaseController
{
				...
				try 
				{
					...
					else
					{
						...
						if ($collection) 
			            {
			            	...
			            	$data[] = $collection->only(['first_name','family_name','email','country_code', 'mobile_no','age','login_type','postal_code','birth_date','gender']);
  ...
  //   }
   ...
   //  }
    ...
    public function getLocationPatientHasOrdination($code,$patient_id)
    {
    ...
    public function getLocationPatientHasOrdinationPuremed($code,$patient_id)
    {
    ...
    public function logout(Request $request) 
    {
        ...
        return self::_sendResult($message,$data);
		...
		if($validator->fails()) {           
		  $errors[] = $validator->errors();
			...
			try {
	                ...
	                if($collection){
	                	$log_id = $collection->id;
						$collection = $collection->only(['first_name','family_name','email','country_code','mobile_no','birth_date','age','road','street_no','postal_code','place','gender']);
			    ...
			    {		
					...
					{*/
		                ...
		                if($collection)
		                {
							...
							//Log::info('edit master data(_oldPatient) line number: 1660 . patient Name :' .$collection->first_name.' '.$collection->family_name);
							$log_id = $collection->id;
							$collection = $new_data->only(['first_name','family_name','email','country_code','mobile_no','birth_date','age','road','street_no','postal_code','place','gender']);
		                	$message = __('api.PATIENT_UPDATE_SUCCESS');
			          		$data[] = $collection;
			          		self::_createLog('editMasterData',$log_id,'info');
			          		$this->ActivityLogModel->addApiLog('Edit Master Data','Edit master data','Update',$oldData,$data[0]);
			...
			try {
					...
					if(!empty($valid_tab_pin)){
						...
						if($valid_tab_pin->setting_value == $tab_pin){
			                ...
			                if(!empty($collection) && ($collection->count() > 0)){
			                	$log_id = $collection->id;
								$collection = $collection->only(['id','old_id','first_name','family_name','email','country_code','mobile_no','birth_date','age','road','street_no','place','postal_code','gdpr','social_security_number','gender']);
					...
					if(!empty($valid_tab_pin)){
						...
						if($valid_tab_pin->setting_value == $tab_pin){
							...
							if(!empty($collectionPatient) && ($collectionPatient->count() > 0)){
								...
								$collection = $collectionPatient->only(['id','old_id','first_name','family_name','email','country_code','mobile_no','birth_date','age','road','street_no','place','postal_code','gdpr','social_security_number','gender']);
	...
	public function smartphoneAppsSetting(Request $request)
	{
        ...
        try
        {
            ...
            if(!empty($collections))
            {
                $status  = true; 
                $message = __('api.DATA_FOUND_SUCCESS');
                $collections->android_review = '0'; //added on 10-jan-24
                $collections->ios_review = '1';  //added on 10-jan-24
                $data  = $collections;
