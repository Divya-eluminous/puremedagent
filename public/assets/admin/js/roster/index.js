$(document).ready(function () 
{
  // alert("sdfdsf");
  var action = ADMINURL + '/roster/getRecords';

  const table = $('#listingTable').DataTable(
    {
      "responsive": true,
      "processing": true,
      "bFilter": true,
      "bInfo": true,
      "bLengthChange": true,
      "serverSide": 'true',
      "ajax": {
            "url": action,
            "data": function (object)  
            {
                object.custom = {
                    "doctor_id" :  $('#doctor_id').val(), 
                    // "appointment_type_id" :  $('#appointment_type_id').val(),
                    "date" :  $('#date').val(),
                    "from_time" :  $('#from_time').val(),
                    "to_time" :  $('#to_time').val(),
                }
            } 
        },
      "columns": [
        { "data": "id", "visible": false },  
        { "data": "doctor_id" },
        // { "data": "appointment_type_id" }, 
        { "data": "date" },
        { "data": "from_time" },
        { "data": "to_time" },
        { "data": "actions" }
      ],

      "aoColumnDefs": [{ "bSortable": false, "aTargets": [0,2,3,4,5] }],
      "lengthMenu": [[20, 25, 50, 100], [20, 25, 50, 100]],
      "aaSorting": [[0, 'DESC']],
      "language": {
            "info": PAGE_SHOW+" _START_ "+PAGE_TO+" _END_ "+PAGE_OF+" _TOTAL_",
            "infoEmpty": PAGE_SHOW+" 0 "+PAGE_TO+" 0 "+PAGE_OF+" 0",
        },
        "oLanguage": {
          "sLengthMenu": "Show _MENU_ Einträge",
          "sSearch": "Suche",
          "oPaginate": {
            "sPrevious": "Vorherige",
            "sNext": "Nächste"
          },
          "infoFiltered": "(gefiltert aus _MAX_ Einträgen)"
        }

    });

  table.on("draw.dt", function (e) 
  {
    setCustomPagingSigns.call($(this));
  }).each(function () {
    setCustomPagingSigns.call($(this));
  });

  function setCustomPagingSigns() {
   $('#date').datepicker({
        dateFormat: 'yy-mm-dd',
    });

    $('#date').mask("9999-99-99");

  }
});

function deleteCollection(element) 
{
  var $this = $(element);
  var action = $this.attr('data-href');

  if (action != '') {
    swal({
      title: deleteContent.title,
      text: deleteContent.text,
      type: "warning",
      showCancelButton: true,
      cancelButtonText: deleteContent.cancel,
      confirmButtonText: deleteContent.confirm,
      confirmButtonClass: "btn-danger",
      closeOnConfirm: false,
      showLoaderOnConfirm: true
    },
      function () {
        axios.delete(action)
          .then(function (response) {
            if (response.data.status === 'success') {
              swal("Success", response.data.msg, 'success');
              $('#listingTable').DataTable().ajax.reload();
            }

            if (response.data.status === 'error') {
              swal("Error", response.data.msg, 'error');
            }

          })
          .catch(function (error) {
            // swal("Error",error.response.data.msg,'error');
          });
      });
  }
}

function doSearch(element)
{
  $('#listingTable').DataTable().draw();
}

function getExcludeDateCollection(element)  
{ 
    var $this = $(element);
    var action = $this.attr('data-href');
   
    $('.card').LoadingOverlay("show", {
        background: "rgba(165, 190, 100, 0.4)",
    });

    axios.post(action)
    .then(response => 
    {  
        $('.card').LoadingOverlay("hide");   
        $('#excludeDateBody').html(response.data.excludeDatesHtml);
        $("#excludeDateModel").modal("show");   
        
    })
    .catch(error =>
    {
        $('.card').LoadingOverlay("hide");
    })
    
    return false;    
}

function excludeDateCollection(element)  
{ 
    var $this = $(element);
    var action = $this.attr('data-href');
 
    $('.modal-body').LoadingOverlay("show", {
        background: "rgba(165, 190, 100, 0.4)",
    });

    axios.post(action)
    .then(response => 
    {  
        $('.modal-body').LoadingOverlay("hide");   
        $("#excludeDateModel").modal("hide");   

        const resp = response.data;

        if (resp.status == 'success') {
            // $this[0].reset();
            toastr.success(resp.msg);
             setTimeout(function () {
               window.location.reload();
            //     window.location.href = resp.url;
             }, 2000);
        } 

        if (resp.status == 'error') {
            toastr.error(resp.msg);
        }
        
    })
    .catch(error =>
    {
        $('.modal-body').LoadingOverlay("hide");
    })

    return false;    
}

// function removeSearch(element)
// { 
//   $('#product-code').val(''),
//   $('#batch-card-no').val(''),
//   $('#listingTable').DataTable().draw();
// }

// processing message in german
$.extend( $.fn.dataTable.defaults, {
    language: {
        "processing": "Verarbeitung..."
    },
});