...
setInterval(function()
 ...
 // $('#appointment_date').mask("9999-99-99"); //commneted by swapnil on 10-jan-23
...
$('#importFindingFrm').validator().on('submit', function (e)
{
    if (!e.isDefaultPrevented()) {
        ...
        $('#importFindingFrm,.model-body').LoadingOverlay("show");
        var comment = $('#comment').val().trim();
        var file = $('input[type="file"]').val().trim();
        if(comment!="" || file!=""){
            axios.post(action, formData)
                .then(function (response) {
                  $('#importFindingFrm,.model-body').LoadingOverlay("hide");
                    const resp = response.data;
                    if (resp.status == 'success') {
                        toastr.success(resp.msg);
                        setTimeout(function(){
                          if (window.location.href.indexOf("?tab=todoList") > -1)
                              var origin   = window.location.href;
                          else
                              var origin   = window.location.href+'?tab=todoList';
                          window.location.href=origin;
                        },500);
                    }
                    //if (resp.status == 'error') {
                      if (resp.status_msg == 'patient-name-match' && resp.status == 'error') {
                        $("#modal-default-upgrade").modal('hide');
                        //console.log(resp.msg_for_todo_list);
                        var msg=(resp.msg_for_todo_list)? resp.msg_for_todo_list:resp.msg;
                        swal({
                          title:'',
                          text: msg+'<br/><a href="'+resp.url+'" target="_blank">Datei zur Kontrolle anzeigen</a>',
                          html: true,
                          showCancelButton: true,
                          allowOutsideClick: true,
                          goToExternal: true,
                          externalUrl: resp.url,
                          confirmButtonText: "TROTZDEM HOCHLADEN",
                          cancelButtonText: "Nicht hochladen",
                          ...
                          function(isConfirm) {
                                ...
                                {
                                  $('#importFindingFrm,.model-body').LoadingOverlay("show");
                                  axios.post(importnew_action, formData).then(function (response) {
                                    $('#importFindingFrm,.model-body').LoadingOverlay("hide");
                                    const resp = response.data;
                                    toastr.success(resp.msg);
                                    setTimeout(function(){
          ...
          alert('Datei oder Notiz ist erforderlich!');
          if (window.location.href.indexOf("?tab=todoList") > -1)
              var origin   = window.location.href;
          else
              var origin   = window.location.href+'?tab=todoList';
...
$("#dashboard_data").css('display','block');
  ...
  /*  //commented below code on 20-sept-24
...
//Added below code on 20-sept-24


/******************changed below code on 20-sept-24****************/
...
/*************changed above code on 20-sept-24*******************************************/
  ...
  resourceLabelDidMount: function(arg) {
    ...
    select: function (date,start, end, jsEvent) {
      ...
      }, 1000); //added on 30-may-25 for #341
    },
  ...
  $('#appointment_date').datepicker({
   ...
   // dateFormat: 'yy-mm-dd', //commented code by swapnil 10-jan-23
     dateFormat: 'dd-mm-yy', //add code by swapnil 10-jan-23
    orientation: "bottom",
    autoclose: true,
    todayHighlight: true,
 ...
 $("#addAppbutton").click(function()
 {
  ...
  $('#frmAppointment').validator().on('submit', function (e) {
   ...
   $('#s_button').removeClass('disabled');
    //if (!e.isDefaultPrevented()) {   // commented this line on 13-sept-22 because app not created
      const $this = $(this);
        ...
        .then(function (response) {
          ...
          if (resp.status == 'success') {
            ...
            $("#time_frame1").val("");
             ...
             console.log('in assis appointment success');
             $(".patient_details").hide();
             $("#suggesstion_patient_div_id").show();
             $("#search_birth_date_div").show();
             // end of added by divya on 29sept22
      ...
      return false;
     //} // commented this line on 13-sept-22 because app not created
  })
  ...
  //console.log("showCreateAppointmentView");
    ...
    .then(response => {
      ...
      const resp = response.data;
        ...
        $('#birth_date').mask("99-99-9999");
       // $('#appointment_date').mask("9999-99-99"); // code commented by swapnil 10-jan-23
       $('#appointment_date').mask("99-99-9999"); // code added by swapnil 10-jan-23
        $('#search_birth_date').mask("99-99-9999");
...
function getDoctorTimeFrames() {
  ...
  function GetServices(appointment_type_id,patient_id,a_id)
  {
    ...
    $("#s_button").prop("disabled", true);//added on 15-jan-26
    ...
    $.ajax({
      ...
      success: function (response)
      {
        ...
        $("#s_button").prop("disabled", false);//added on 15-jan-26
      }
    });
  }
...
function completedNewOld(element,p_id,type,coll_type)
{
  var element = element;
  var $this = $(element);
  swal({
...
function completedNew(element,p_id,type,coll_type,old_date_id='',hd_patient_id='',hd_date='')
{
  var element = element;
  var $this = $(element);
  swal({
    ...
    function ()
    {
      $.ajax({
        ...
        {
          'p_id': p_id,
          'old_date_id':old_date_id,
          'hd_patient_id':hd_patient_id,
          'hd_date':hd_date
        },
        success: function (response)
        {
          ...
          {
            $("#totdoList-tab").html(
                '<button id="btn_todoList_cnt" type="button" class="btn btn-primary btn-cnt">' +
                    response.count +
                    "</button>" +
                    todolist_title
            );
...
function showSendFinding(element,type,id,old_id,send_type)
{
  ...
  // setTimeout(function(){
  //   completedNew(element,id,'completed','finding');
  // }, 100);
}
...
$(document).on('click','.copybutton',function(){
  ...
  // var copyText = document.getElementById(name);
  // //console.log(name); console.log(copyText);
  var copyText = $(this).parent().next().find("input");
  copyText.select();
  //copyText.setSelectionRange(0, 99999);
  document.execCommand("copy");
  toastr.success(copy_success_msg);
...
$(document).on('click','.copyPatient',function(){
  //console.log($(this).next('input:hidden:first').val());
  var pName =$(this).next('input:hidden:first').val()
  var $temp = $("<input>");
  $("body").append($temp);
  $temp.val(pName).select();
  document.execCommand("copy");
  $temp.remove();
  toastr.success(pName+' - '+copy_success_msg);
});